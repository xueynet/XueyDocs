<template><h1 id="data-backup-数据库备份命令"><a class="header-anchor" href="#data-backup-数据库备份命令">#</a> data backup 数据库备份命令</h1>
<p>数据库备份命令，用于备份数据库结构以及数据至指定文件夹中。</p>
<div class="custom-container tip"><p class="custom-container-title">TIP</p>
<p>数据库备份命令不仅可以用于备份 XYCMS 系统数据，还可用于任何需要将数据库备份至文件的场合。</p>
</div>
<h3 id="用法"><a class="header-anchor" href="#用法">#</a> 用法</h3>
<div class="language-bash ext-sh line-numbers-mode"><pre v-pre class="language-bash"><code>xycms data backup
    -d, --directory<span class="token operator">=</span>VALUE
    --includes<span class="token operator">=</span>VALUE
    --excludes<span class="token operator">=</span>VALUE
    --max-rows<span class="token operator">=</span>VALUE
    -h, --help
</code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br></div></div><p>可以设置可选参数控制备份选项。</p>
...
<td style="text-align:left">指定保存备份文件的文件夹名称</td>
</tr>
<tr>
<td style="text-align:left">--includes</td>
<td style="text-align:left"></td>
<td style="text-align:left">指定需要备份的表，多个表用英文逗号隔开，默认备份所有表</td>
</tr>
<tr>
<td style="text-align:left">--excludes</td>
<td style="text-align:left"></td>
<td style="text-align:left">指定需要排除的表，多个表用英文逗号隔开</td>
</tr>
<tr>
<td style="text-align:left">--max-rows</td>
<td style="text-align:left"></td>
<td style="text-align:left">指定需要备份的表的最大行数，默认备份所有数据</td>
</tr>
<tr>
...
<p>备份命令将读取当前目录下的 xycms.json 文件，从中获取数据库连接字符串，进而将数据库备份到 backup 文件夹中。</p>
<p>备份命令目前还不支持增量备份，只支持完全备份，即备份命令将把数据库中所存的所有数据备份至文件夹中，如果数据库中数据量太大，请不要频繁使用备份命令，以免影响数据库性能。</p>
<p>备份命令默认将数据库数据备份至当前目录下的 <code>backup/{当前日期}</code> 文件夹中，如果要备份至指定文件夹，请设置 <code>-d</code> 或者 <code>--directory</code> 参数指定文件夹名称。</p>
<p>例如我们希望备份指定的 XYCMS 系统数据，找到系统所在文件夹路径，将命令行切换到此文件夹，并运行备份命令：</p>
...
<p><code>xycms backup -d mydir</code></p>
